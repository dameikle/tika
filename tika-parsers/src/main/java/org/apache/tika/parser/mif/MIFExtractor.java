...
public class MIFExtractor {
    ...
    static void parse(AutoDetectReader reader, ContentHandler handler) throws IOException, SAXException {
        ...
        while ((line = reader.readLine()) != null) {
            if (line.contains("<") && !line.contains(">")) {
                String singleTagReg = "<(\\S*).*";
                Pattern singleTagPattern = Pattern.compile(singleTagReg);
                Matcher matcher = singleTagPattern.matcher(line.trim());
                if (matcher.matches()) {
                    if (!parents.empty()) {
                        currentTag = new Tag();
    ...
    /**
     * Helper Class to Model a MIF Tag
     */
